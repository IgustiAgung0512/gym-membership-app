...
class MemberController extends Controller
{
    ...
    /**
     * Kasir menautkan kartu RFID fisik untuk member pendaftaran online yang datang mengambil kartu
     */
    public function assignRfid(Request $request, Member $member)
    {
        ...
        $existing = \App\Models\RfidCard::with('member.user')->where(function ($q) use ($uid, $cleanUid, $trimmedColonUid) {
            $q->where('uid', $uid)
        ...
        if ($member->rfidCard) {
            ...
            \App\Models\RfidCard::updateOrCreate(
                ['uid' => $uid],
                [
                    'member_id' => $member->id,
                    'status' => 'assigned',
        ...
        return redirect()->route('cashier.members.index')->with('success', "Kartu RFID {$uid} berhasil diberikan & ditautkan ke member {$member->user->name}!");
    }
}